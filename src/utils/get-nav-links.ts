...
export function getPreviousAndNextLinks(navLinks: NavLink[], Astro: Readonly<AstroGlobal>): PreviousAndNext {
  const index = navLinks.findIndex((nav) => Astro.url.pathname.replace(/\/$/, '').endsWith(nav.link));
	return {
		previous: index > 0 ? navLinks[index - 1] : undefined,
		next: index !== -1 && index < navLinks.length - 1 ? navLinks[index + 1] : undefined,
	};
}
