...
<CopyBody client:load>
  ```bash composer require treblle/treblle-symfony ```
</CopyBody>
...
<CopyBody client:load>
  ```yaml treblle: project_id: "%env(TREBLLE_PROJECT_ID)%" api_key:
  "%env(TREBLLE_API_KEY)%" debug: false masked: - password - api_key - secret
...
<CopyBody client:load>
  ```yaml Treblle\Contract\RequestDataProvider:
  '@App\Utils\AcmeRequestDataProvider' ```
</CopyBody>
